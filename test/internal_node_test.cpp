...
{
  // prepare an internal node via a leaf node
  auto target_node = InternalNode::CreateEmptyNode(kDefaultNodeSize);
...
TEST_F(InternalNodeFixture, NeedSplit_FilledNode_SplitRequired)
{
  ...
  WriteNullKey(tmp_leaf_node, 10);
  auto target_node = BitCast<InternalNode*>(tmp_leaf_node);
